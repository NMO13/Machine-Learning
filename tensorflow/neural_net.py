...
tr_features = np.array(list(map(lambda x: x.flatten(), training_data[:, 0])))
tr_labels = np.array(list(map(lambda x: x.flatten(), training_data[:, 1])))
...
l = tf.losses.mean_squared_error(Y, a)
#optimizer
train_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(l)

#compare predicted value from network with the expected value/target
correct_prediction = tf.equal(tf.arg_max(a, 1), tf.arg_max(Y, 1))
